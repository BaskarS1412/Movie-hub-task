...
row.setAttribute(
  "class",
  "row h-auto justify-content-center align-items-center"
);
...
card.setAttribute(
  "class",
  "card col-11 col-md-10 col-lg-8 h-auto d-flex justify-content-center align-content-center my-5"
);
...
heading1.innerHTML = `<span class="px-2 bg-warning border rounded pill"><i class="fa-solid fa-film"></i> Cinematography!!!</span>`;

const description = document.createElement("p");
description.setAttribute("class", "text-center fw-normal");
description.innerText = `Movies can be fictional or based on real events, and they cover a wide range of genres, including drama, comedy, action, horror, science fiction, and more. They are a popular and influential art form, capable of entertaining, inspiring, and impacting audiences on emotional and intellectual levels.`;
...
filmNameLabel.setAttribute(
  "class",
  "col-12 col-md-4 col-lg-4 col-form-label text-nowrap text-center fw-bold"
);
...
movieInfoDiv.setAttribute(
  "class",
  "mx-auto filmDiv justify-content-center align-content-center col-11 py-3 my-3 bg-warning position-relative"
);
...
filmTitle.setAttribute(
  "class",
  "text-uppercase text-center h4 text-wrap h-auto py-3"
);

let moviePoster = document.createElement("div");
moviePoster.setAttribute(
  "class",
  "movie-poster py-3 d-flex justify-content-center align-items-center"
);
...
async function getMovieDetails() {
  try {
    ...
    const response = await fetch(
      `https://www.omdbapi.com/?apikey=${apiKey}&t=${movieName}&y=${movieYear}&plot=full`
    );
    if (!response.ok) {
    ...
    if (data.Response === "True") {
      ...
      alert(
        `The input field must have a valid Movie Title!\nEmpty Field or Typo Not Accepted`
      );
    }
  } catch (error) {
    if (error.message === "TypeError: Failed to fetch") {
      alert(
        "There's a connectivity problem. Please check your internet connection."
      );
    } else {
